resource "azurerm_virtual_machine_scale_set_extension" "vmss_init" {
  ...
  settings = jsonencode({
    ...
    # commandToExecute = "powershell -ExecutionPolicy Unrestricted -Command \"Install-WindowsFeature Telnet-Client; Install-WindowsFeature Web-Server -IncludeManagementTools; Import-Module WebAdministration; Start-Sleep -Seconds 30; $fqdn='${var.api_dns_name}.${var.private_dns_zone_name}'; $cert = Get-ChildItem Cert:\\LocalMachine\\My | Where-Object { $_.DnsNameList -like '*${var.private_dns_zone_name}*' } | Select-Object -First 1; if ($cert) { New-WebBinding -Name 'Default Web Site' -Protocol https -Port 443 -HostHeader $fqdn -SslFlags 1;  New-Item IIS:\\SslBindings\\0.0.0.0!443!$fqdn -Thumbprint $cert.Thumbprint -SSLFlags 1; iisreset }\""
    # commandToExecute = "powershell -ExecutionPolicy Unrestricted -Command \"Install-WindowsFeature Telnet-Client; Install-WindowsFeature Web-Server -IncludeManagementTools; Import-Module WebAdministration; Start-Sleep -Seconds 30; $cert = Get-ChildItem Cert:\\LocalMachine\\My | Where-Object { $_.DnsNameList -like '*${var.public_domain}*' } | Select-Object -First 1; if ($cert) { $internalFqdn='${var.api_dns_name}.${var.private_dns_zone_name}'; $publicFqdn='${var.api_dns_name}.${var.public_domain}'; New-WebBinding -Name 'Default Web Site' -Protocol https -Port 443 -HostHeader $internalFqdn -SslFlags 1; New-Item IIS:\\SslBindings\\0.0.0.0!443!$internalFqdn -Thumbprint $cert.Thumbprint -SSLFlags 1; New-WebBinding -Name 'Default Web Site' -Protocol https -Port 443 -HostHeader $publicFqdn -SslFlags 1; New-Item IIS:\\SslBindings\\0.0.0.0!443!$publicFqdn -Thumbprint $cert.Thumbprint -SSLFlags 1; iisreset }\""
